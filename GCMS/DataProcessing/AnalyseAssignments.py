...
retention_times = {}
mass_spectra = {}
for ai,c in enumerate([6]):

    # get data according to assigned carbon chain length
    n_inds = [i for i, x in enumerate(compound_labels) if x.count('C') == c]
    n_mat = scaled_data_mat[n_inds]
    wt_mat = integral_mat[n_inds]
    local_rts = data_mat[n_inds,0]

    # local colour scheme and compound labels
    n_clrs = clrs[n_inds]
    comps = [x for x in compound_labels if x.count('C') == c]
    ...
    pca = PCA(n_components = int(len(n_inds)/3))
    dim_red = pca.fit_transform(n_mat)
    print(dim_red.shape)
    # try to spread out the data a bit more using LDA
    lda = LinearDiscriminantAnalysis(n_components=n_clust-1)
    X_r2 = lda.fit(dim_red, comps).transform(dim_red)
    print(X_r2.shape)
    ...
    for z in range(n_clust):
        ...
        loc_comps = [comps[i] for i in inds]
    ...
    plt.savefig(collated_folder/'C{}_GCMS_{}.png'.format(c,cluster_tag), dpi = 600)
    plt.close()
    print('C{} complete'.format(c))
